...
maybe_fan_out_activity(ActivityId, Context) ->
    case m_rsc:is_a(ActivityId, activity_create, Context) of
        false ->
            ok;
        true ->
            case m_edge:object(ActivityId, has_target, 1, Context) of
                undefined ->
                    ok;
                TargetId ->
                    Actors = sets:from_list(m_edge:objects(ActivityId, has_actor, Context)),
                    To = sets:from_list(m_edge:objects(ActivityId, has_audience_to, Context)),
                    Cc = sets:from_list(m_edge:objects(ActivityId, has_audience_cc, Context)),
