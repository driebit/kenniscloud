...
{% block title %}{% if id %}Bron bewerken{% else %}Bron toevoegen{% endif %}{% endblock%}

{% block page_class %}ginger-edit{% endblock %}

{% block container %}
    {% with "reference" as category %}
...
{% block header %}
    ...
    <header class="ginger-edit__header">
        <nav class="ginger-edit__nav">
            {# button die een popup geeft dat je werk niet meteen zichtbaar is maar dat het door de redactie wordt bekeken dan de pagina opslaat en dan een redirect doet #}

            {% button class="btn--save" text=_"Plaatsen" title="Plaatsen"
    ...
    {% if m.acl.user %}
        ...
        <div class="row page-ginger_edit_content_row_class">
            <div class="col-sm-8 col-md-8">
                {% include "_ginger_edit.reference.tpl" isnew=`true` %}
            </div>

            <div class="col-sm-4 col-md-4">
                {% include "_aside_ginger_edit.tpl" author=q.auteur %}
            </div>
        </div>
        </form>
    {% else %}
