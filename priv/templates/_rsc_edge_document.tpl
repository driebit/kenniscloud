{# Show a thumbnail with an unlink option. Used on the edit page to add 'hasdocument' connections #}
...
<li id="{{ #unlink_wrapper }}" class="{% if not object_id.is_published %}unpublished{% endif %}">
    {% if has_cropcenter %}
        ...
        <figure class="do_cropcenter"
                data-cropcenter='{{
                    %{
                        "hiddenInput": "#crop_center_" ++ object_id,
                        "editable": false
                    }|escape
                }}'
    ...
    action={
        ...
        undo_action={
            ...
            postback={
                reload_media
                rsc_id=id
                div_id=["links-",id|make_list,"-document"]
            }
...
{% wire
    ...
    action={
        dialog_edit_basics
        edge_id=edge_id
        template="_rsc_edge_document.tpl"
    }
%}
