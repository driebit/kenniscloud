{% with not id or id.is_editable as is_editable %}
    <div class="row">
        <div class="form-group col-xs-12">
            <label class="control-label" for="{{ #title }}{{ lang_code_for_id }}">Titel</label>
            <p class="control-label-summary">Formuleer een korte, prikkelende titel of stel een vraag</p>
    ...
    {% live
        topic={object id=id predicate=`about`}
        template="contribution/add-about.tpl"
        id=id
        max=1
    %}

    {% live
        topic={object id=id predicate=`hasbanner`}
        template="contribution/add-banner.tpl"
        id=id
        max=1
        collab_group=q.kennisgroep|to_integer
    %}
